...
#SBATCH --job-name="ALN-Interleave"
#SBATCH --exclusive
#SBATCH --mem=125G
#SBATCH -w penguin
#SBATCH --time=40:00:00
#SBATCH --partition=p_hpca4se 
#SBATCH -o /scratch/077-hpca4se-bioinf/jlenis/logs/q1-2016/interleave/aln_%j.log 
...
original_path=`pwd`
mapper_path="$main_dir/software/mappers/bwa-0.7.12_penguin"
cd $mapper_path
...
echo $OUT_T2
perf stat -e $counters -a --per-socket  -c -r $repeat $socket_1 bwa aln -t 8 $local_index_path/HG_index_BWA_default/hsapiens_v37.fa $local_dataset_path/$IN.fastq > $results_path/$OUT_T2.sam
...
echo $OUT_T3
perf stat -e $counters -a --per-socket  -c -r $repeat $socket_2 bwa aln -t 16 $local_index_path/HG_index_BWA_default/hsapiens_v37.fa $local_dataset_path/$IN.fastq > $results_path/$OUT_T3.sam
...
echo $OUT_T4
perf stat -e $counters -a --per-socket  -c -r $repeat $socket_4 bwa aln -t 32 $local_index_path/HG_index_BWA_default/hsapiens_v37.fa $local_dataset_path/$IN.fastq > $results_path/$OUT_T4.sam
...
echo $OUT_T5
perf stat -e $counters -a --per-socket  -c -r $repeat $socket_6 bwa aln -t 48 $local_index_path/HG_index_BWA_default/hsapiens_v37.fa $local_dataset_path/$IN.fastq > $results_path/$OUT_T5.sam
...
perf stat -e $counters -a --per-socket  -c -r $repeat $socket_all bwa aln -t 64 $local_index_path/HG_index_BWA_default/hsapiens_v37.fa $local_dataset_path/$IN.fastq > $results_path/$OUT_T6.sam
